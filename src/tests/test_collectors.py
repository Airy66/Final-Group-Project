...
from ecommerce_price_monitor.collectors.walmart_collector import WalmartCollector
...
class TestWalmartCollector:
    """Test WalmartCollector class."""

    def test_walmart_collector_initialization(self):
        """Test WalmartCollector initialization."""
        collector = WalmartCollector()
        assert collector.platform_name == "Walmart"
        assert collector.base_url == "https://www.walmart.com"
        assert collector.search_url == "https://www.walmart.com/search"

    def test_extract_product_id_valid_urls(self):
        """Test extracting product ID from valid Walmart URLs."""
        collector = WalmartCollector()
        ...
        test_urls = [
            ...
            ("https://www.walmart.com/ip/1756765288", "1756765288"),
            ("https://www.walmart.com/product/1756765288", "1756765288"),
            ("https://www.walmart.com/ip/test?itemId=1756765288", "1756765288"),
        ]
    ...
    def test_extract_product_id_invalid_urls(self):
        """Test extracting product ID from invalid Walmart URLs."""
        collector = WalmartCollector()
    ...
    def test_parse_price_valid(self):
        """Test parsing valid Walmart price strings."""
        collector = WalmartCollector()

        test_prices = [
            ("$19.99", 19.99),
            ("current price $1,234.56", 1234.56),
            ("Now $149.00 Was $179.00", 149.00),
            ("19.99", 19.99),
        ]
    ...
    def test_get_product_details_from_article_style_html(self):
        """Test product detail extraction from h1 and itemprop price HTML."""
        collector = WalmartCollector()
        html = """
        <html>
            <body>
                <h1>AT&amp;T iPhone 14 128GB Midnight</h1>
        ...
        collector._make_walmart_request = Mock(return_value=mock_response)

        product = collector.get_product_details(
            "https://www.walmart.com/ip/AT-T-iPhone-14-128GB-Midnight/1756765288"
        )

        assert product is not None
        assert product.platform == "Walmart"
        assert product.product_id == "1756765288"
        assert product.name == "AT&T iPhone 14 128GB Midnight"
        assert product.price == 629.00
        assert product.currency == "USD"

    def test_get_product_details_from_json_ld(self):
        """Test product detail extraction from JSON-LD structured data."""
        collector = WalmartCollector()
        html = """
        <html>
            <script type="application/ld+json">
            {
                ...
                "@type": "Product",
                "name": "Straight Talk Apple iPhone 15 Pro",
                "image": "https://i5.walmartimages.com/test.jpeg",
        ...
        product = collector.get_product_details(
            "https://www.walmart.com/ip/Straight-Talk-Apple-iPhone-15-Pro/5060213862"
        )

        assert product is not None
        assert product.product_id == "5060213862"
        assert product.name == "Straight Talk Apple iPhone 15 Pro"
