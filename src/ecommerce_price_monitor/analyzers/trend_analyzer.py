...
class TrendAnalyzer(BaseAnalyzer):
    ...
    def analyze(
        ...
        **kwargs
        ...
        try:
            ...
            raise ValueError(f"Trend analysis failed: {e}")  
    ...
    def _detect_peaks_troughs(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for peak_idx in peaks:
            peak_data.append({
                'timestamp': pd.Timestamp(timestamps[peak_idx]).isoformat(),
        ...
        for trough_idx in troughs:
            trough_data.append({
        'timestamp': pd.Timestamp(timestamps[trough_idx]).isoformat(),
    ...
    def _predict_trend(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        recent_data = df.tail(min(10, len(df))).reset_index(drop=True)
        prices = recent_data['price'].to_numpy()
        timestamps = (pd.to_numeric(recent_data['timestamp']) / 1e9).to_numpy()
